...
export const transferOwnership = async () => {
  ...
  const automate = await ethers.getContractAt(
    ownableInterface,
    automateAddress
  );
  ...
  // const automate = await ethers.getContractAt("EIP173Proxy", automateAddress);
  // const ownerAddress = await automate.proxyAdmin();

  console.log("Current Owner: ", ownerAddress);
  console.log("New Owner: ", newOwnerAddress);

  await automate.transferOwnership(newOwnerAddress);
};
