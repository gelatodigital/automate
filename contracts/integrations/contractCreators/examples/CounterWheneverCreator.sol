...
import {WheneverCreator} from "../WheneverCreator.sol";

/**
 * @notice
 * Example of creating a task which Gelato
 * will constantly simulate and executes
 * when the function call will not revert.
 */
// solhint-disable not-rely-on-time
// solhint-disable no-empty-blocks
contract CounterWheneverCreator is WheneverCreator {
    ...
    function createTask() external payable {
        ...
        bytes32 id = _createTask(address(this), execData, address(0));
    ...
    function increaseCount(uint256 _amount) external onlyDedicatedMsgSender {
        require(block.timestamp >= lastExecuted + INTERVAL);
