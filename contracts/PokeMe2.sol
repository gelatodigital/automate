...
contract PokeMe2 is ReentrancyGuard, Gelatofied {
  ...
  function getTaskId(address _execAddress, bytes4 _selector)
    public
    pure
    returns (bytes32)
  {
  ...
  {
    uint256 _length = _createdTasks[_callee].length();
    bytes32[] memory taskIds = new bytes32[](_length);
  ...
  {
    uint256 _length = _createdTasks[_callee].length();
    address[] memory creditTokens = new address[](_length);
