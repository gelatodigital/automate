...
contract TaskTreasuryAccounting is Ownable, ReentrancyGuard {
    ...
    /// @notice Chosses token with highest balance to be used for payment
        ...
        )
    {
        ...
        for (uint256 x; x < creditTokensLength; x++) {
            if (x >= creditTokensOld.length) {
                treasuryToCheck = address(this);
                creditTokensToCheck = creditTokensNew;
            }

            address creditToken = creditTokensToCheck[ptr];
            ...
            if (treasuryToCheck == oldTreasury && ptr >= creditTokensOld.length)
                ptr = 0;
        }
    }
}
